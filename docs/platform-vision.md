...
## Onboarding: four things get generated, and stay editable afterward
...
who or what they're looking for. From that description, the platform's AI generates four
things, each of which is fixed/global on the real-estate platform today and becomes
generated-per-Project instead — and **all four stay editable afterward**, even once the
Project already has real data in it, not locked in after onboarding:

1. **Entity fields** — what a tracked record actually looks like (today, a fixed set of
   real-estate columns: domain, HQ, AUM, target markets). Becomes whatever fields this
   Project actually needs, generated from the description, and editable later via an
   ongoing field editor — add, remove, or rename a field any time, even with firms already
   populated.
2. **Classification** — the qualification criteria that decide whether something is a fit
   (today, a single hardcoded global AUM band — "Within Mandate" — this becomes whatever
   criteria actually matter for this Project, generated from the description, editable
   later).
3. **Taxonomy** — the classification tree used to organize and tag what's found (the same
   ...
   optional override — already editable/regeneratable after confirmation, same as today).
4. **CRM Stages** — the pipeline this Project's outreach/relationship-building process moves
   through (today, one fixed global 13-stage list — this becomes a generated stage sequence
   fitted to what the user described, freely rename/reorder/add/replace afterward).

**Everything else stays exactly as built.** The research/enrichment engine, the reporting
engine, Messages, Tasks, Settings, and every other module described in the current-state
documentation are unchanged — it's specifically these four configuration surfaces moving
from "one fixed global answer" to "generated per
...
engine, reporting — all of it runs exactly as built, now operating through the fields,
classification, taxonomy, and stages the user just defined. A Project sourcing institutional
investment managers keeps AUM as a field, an AUM-based classification, and a capital-raising
