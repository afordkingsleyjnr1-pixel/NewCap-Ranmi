 ...
 * the exact same CRM stage transitions per entity instead of re-implementing
...
export async function sendOutreachToFirm(params: {
  ...
  const stage = await prisma.entityStage.findUniqueOrThrow({ where: { entityId: params.entityId } });
  if (stage.stage === "do_not_contact") {
    throw new OutreachError("This entity is marked Do Not Contact. Outreach is blocked.", "DO_NOT_CONTACT");
  }
  ...
  let thread =
    params.kind !== "email"
      ? await prisma.emailThread.findFirst({ where: { entityId: params.entityId, contactId: params.contactId ?? undefined }, orderBy: { createdAt: "desc" } })
      : null;
  ...
  try {
    ...
    await prisma.activityLog.create({
      data: { entityId: params.entityId, contactId: params.contactId, type: "email_sent", body: `${actionLabel} sent: "${params.subject}"`, createdById: params.userId },
    });

    let nextStage: CrmStage;
    if (params.kind === "email") {
      nextStage = "email_sent";
      await completePendingTask(params.entityId, entity.name, "send_email");
    } else if (params.kind === "follow_up") {
      nextStage = "follow_up_sent";
      await completePendingTask(params.entityId, entity.name, "send_follow_up");
    } else {
      nextStage = "term_sheet_sent";
      await completePendingTask(params.entityId, entity.name, "send_term_sheet");
    }

    await prisma.entityStage.update({ where: { entityId: params.entityId }, data: { stage: nextStage, stageChangedAt: new Date() } });
    await prisma.activityLog.create({
      data: { entityId: params.entityId, type: "stage_change", body: `Stage changed to ${nextStage}`, createdById: params.userId },
    });
