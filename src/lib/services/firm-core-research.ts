...
// since all four need the same web presence for the same entity anyway.
...
3. CLASSIFICATION: Read the entity's site (About, Investment Strategy, Investment Approach, Portfolio, Sectors, Industries, Capabilities, Funds, Platform, Lending, Credit, Real Estate, Infrastructure, Team, Investor Relations, News, Transactions, Case Studies, Portfolio Companies) and map what they actually do — not marketing language — onto the taxonomy provided below (given as a separate reference block). Classify only what the entity ACTIVELY does today: currently invests in, currently lends into, has a dedicated fund or established platform for, or is demonstrated by multiple transactions/portfolio companies/case studies. Do NOT include something mentioned only in thought-leadership content, described as a future opportunity, based on one isolated transaction, or belonging to a portfolio company rather than the manager itself. This rule overrides all others. Use ONLY parent/child names from the taxonomy — never invent one.

4. CONTACTS: Identify the best-fit capital-raising contact(s) at the entity, only if you found a real official domain for them. The single highest priority: find anyone whose title contains the words "Capital Markets", "Capital Introductions", or "Capital Formation" — at ANY seniority level (Analyst, Associate, VP, Director, Head, Managing Director, Partner, etc. all count equally). A "Director of Capital Markets" outranks a "Head of Investor Relations" who has none of those three phrases in their title. Only if nobody at the entity has one of those three phrases, fall back to the next-best capital-raising contact: Head of Investor Relations > Head of Business Development / BD > Fundraising lead > any senior IR/BD-adjacent title. Include alternate/secondary contacts after the primary one if you find other people doing the same kind of work. Return at most 3. Never invent a name — if none can be found confidently, return an empty array.
...
// once Prisma tries to write it, aborting the whole entity add. Rather than
